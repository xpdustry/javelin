...
public class JavelinServer implements Closeable {
  ...
  private static final class JavelinWebSocketServer extends WebSocketServer {
    ...
    public JavelinWebSocketServer(final int port, final int workers, final @NotNull Authenticator authenticator) {
    ...
    public void onMessage(final @NotNull WebSocket conn, final @NotNull ByteBuffer message) {
      try (final var input = new ByteBufferInput(message); final var output = new Output(1024)) {
